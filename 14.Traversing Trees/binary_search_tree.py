...
class NodeBST(NodeBT):
    ...
    def _add_next_node(self, value, level_hear=2):
        new_node = NodeBST(value, level_hear)
        if value > self.value:
            self.right = self.right and self.right._add_next_node(value, level_hear+1) or new_node
        elif value < self.value:
            self.left = self.left and self.left._add_next_node(value, level_hear+1) or new_node
        else:
